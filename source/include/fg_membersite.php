...
class FGMembersite
{
    ...
    function UserType()
    {
	return isset($_SESSION['t'])?$_SESSION['t']:'';
    }
    function UserUid()
    {
    ...
    function CheckLoginInDB($email,$password)
    {
        ...
        $pwdmd5 = md5($password);
        $qry = "Select name,uid,type,email from $this->tablename where email='$email' and password='$pwdmd5'";
    ...
    function ChangePasswordInDB($user_rec, $newpwd)
    {
        ...
        $qry = "Update $this->tablename Set password='".md5($newpwd)."' Where  uid=".$user_rec['uid']."";
	...
	$host = "ssl://smtp.gmail.com";
        $port = "465";
        $username = "helthtoken949@gmail.com";
        $password = "ht_12345678";
        
        $from ="helthtoken949@gmail.com";
       
        $subject = "Welcome to ".$this->sitename;
	$bodystart = "Hello ".$user_rec['name']."\r\n\r\n".
	...
	$bodystart = "A new user registered at ".$this->sitename."\r\n".
        "Name: ".$user_rec['name']."\r\n".
	...
	$host = "ssl://smtp.gmail.com";
        $port = "465";
        $username = "healthtoken949@gmail.com";
        $password = "ht_12345678";
        
        $from ="healthtoken949@gmail.com";
	...
	$validator->addValidation("mob","req","maxlen=10","Please provide a valid mobile number");
	$validator->addValidation("age","req","Please provide age");
	$validator->addValidation("city","req","Please provide city");
	$validator->addValidation("state","req","Please provide state");
	$validator->addValidation("country","req","Please provide country");
	$validator->addValidation("address","req","maxlen=250","Please provide valid address");
	...
	$formvars['mob'] = $this->Sanitize($_POST['mob']);
    ...
    function IsFieldUnique($formvars,$fieldname)
    {
        ...
        $qry = "select name from $this->tablename where $fieldname='".$field_val."'";
        $result = mysql_query($qry,$this->connection);   
        if($result && mysql_num_rows($result) > 0)
        {
            return false;
	...
	$result = mysql_query("Select uid from $this->tablename where email='".$formvars['email']."'",$this->connection);
	$row = mysql_fetch_assoc($result);
	$insert_query = 'insert into pa(
                name,
                mob,
                age,
		sex,
		...
		uid
                ...
                (
                "' . $this->SanitizeForSQL($formvars['name']) . '",
		"' . $this->SanitizeForSQL($formvars['mob']) . '",
